...
4. **Stale-response overwrite (no generation token)** — because fetch is unawaited and un-guarded, a slow first fetch can land *after* a faster second fetch and overwrite the newer data with older data. Partially guarded since 4.11 (F-RT4/F-RT5/F-RT10): `BucketState` skips a snapshot whose version was bumped mid-fetch (bucket_state.dart:80-91, 130-162), `HomepageState` refetches a miss-you total observed stale (homepage_state.dart:64-68), and `CheckpointMixin` write-order tokens drop an older-observed checkpoint write (checkpoint_mixin.dart). Residual: the *in-memory* collection replace on other states (mood/game/drive snapshot sockets) still has no request-id check — a slow fetch landing after a faster one replaces newer data, though checkpoints (and hence cold-start correctness) are protected. Confidence: MEDIUM (possible, not observed in tests).
...
- Logout: `logout()` (auth_state.dart:695-711) signs out Supabase, clears checkpoints (`CacheService.clearAll`), clears StorageService, drops all auth/partner fields, then invokes `onClearFeatureStates` — registered by `RealtimeSyncScope` (realtime_sync_scope.dart) to reset the six feature states (Homepage/Mood/Bucket/Game/Drive/Profile) **in memory after** storage/checkpoints are cleared, so the next session starts empty and the first `init()` re-fetches instead of showing the previous user's data. `LogoutUtils.performLogout` (logout_utils.dart:39-48) additionally pops to the LoginScreen. The wipe path (`profile_screen.dart:134-150`) clears the same six states for the same underlying reason.
- Session expiry path: any 401 during API calls triggers `ApiService.onSessionExpired = logout` (auth_state.dart:81-86; api_service 401 handling documented in architecture scope), which runs `AuthState.logout`. The returned synthetic `"401"` code is in the `requiresReauth` set (`ErrorCodes.httpUnauthorized`, error_codes.dart), so the same result also opens the reauth dialog (error_handler.dart:145-146) -> redirect to `/login`; the other `requiresReauth` codes are `AUTH-FAIL-001/002/003/006`. The global callback and the reauth dialog both funnel into the same `logout()`.

## Partnership state

- Two overlapping objects: `AuthState` holds partner *identity* (`_partnerId`, `_partnershipId`, `_partnerDisplayName`, auth_state.dart:39-43) while `PartnerState` holds the full `_partnershipInfo` (partner + pendingRequests + anniversary, partner_state.dart:13-20). `RealtimeSyncScope` treats them as complementary sources and must merge them: `resolvedPartnershipId = authState.partnershipId ?? partnerState.partnershipInfo?.partnershipId` (realtime_sync_scope.dart:46-47); `partnerId = authState.partnerId ?? partnerState.partnershipInfo?.partner?.id` (lines 51-52).
- Refresh paths that write both: `RefetchRealtimeHandler.handle` (lib/core/realtime/refetch_realtime_handler.dart) calls `BaseRepository.clearPartnershipCache()` then `Future.wait([partnerState.refreshFromRealtime(), authState.refreshPartnershipFromRealtime()])`. `authState.refreshPartnershipFromRealtime` (auth_state.dart:447-477) re-derives fields from `getPartnership()` and clears partner if status isn't `accepted`. `PartnerState.refreshFromRealtime` (partner_state.dart:30-36) re-applies the whole `PartnershipResponse` and re-saves partner/partnershipId to StorageService (partner_state.dart:38-49).
...
- `addMissYou` is an optimistic counter bump + unawaited save (homepage_state.dart:64-68). Realtime `insert` events call `addMissYou()` directly (handlers/miss_you_realtime_handler.dart), i.e. the same code path used for the user's own action — an insert trigger on the user's own row double-counts if the optimistic bump already happened.
...
- `updateMood` (mood_state.dart:233-288): optimistic mutate **in-memory only** — mood, recentEmojis, timeline + notify (lines 238-259); the cache is never written before the server confirms, so a failed/killed update cannot persist a phantom `id:0,userId:0` timeline entry or a rejected emoji (F-SC14 resolved). On success the optimistic placeholder is replaced with the server-confirmed entry, then mood/recentEmojis/timeline are persisted + checkpoint (lines 261-276); on error the **full** rollback restores mood, timestamp, `_recentEmojis`, timeline and the timeline offset, with the cache already reflecting the pre-update state (F-SM12 resolved).
...
- Cache debounce: `_scheduleCacheSave` 2s Timer → `_flushCache` (bucket_state.dart:193-197). Flushes are serialized through `CacheWriteQueue`, and `dispose()` / `didChangeAppLifecycleState` (hidden/paused/detached) flush the pending write immediately (lines 18-43) — the debounce no longer risks losing the last 2s of changes on dispose or app-pause (F-SC13 resolved).
...
- `updateDisplayName`, `uploadProfilePhoto`, `updateAnniversaryDate`, `wipeAppData` (lines 84-174): all `runSafe`-wrapped, optimistic in-place copy on the user profile + StorageService rewrites. `updateDisplayName` returns `Future<bool>` and **throws the failed `ResultWrapper`** inside `runSafe` so a non-success fails loudly instead of being swallowed by `handleResult`. `isUploading` handled with explicit notify (lines 127-150) but `runSafe(showLoading: false)` — so uploading does not trigger the BaseState loading path.
- Realtime-refreshed (F-SM6): `user_profiles` is subscribed via `RefetchRealtimeHandler` (lib/core/realtime/refetch_realtime_handler.dart, 150 ms debounce, update-only via its `isRelevant` wiring); self/partner profile `UPDATE`s trigger `loadProfile(force: true)` on `ProfileState` plus partnership/partner refreshes. Regular (non-wipe, non-realtime) loads still depend on the homepage `_loadData` or explicit `loadProfile(force: true)`. Note `profile_screen.dart` does NOT use TabScreenMixin either.
...
| Realtime insert/update/delete | per-feature handlers debounced 120-150ms → state methods (`lib/core/realtime/handlers/`) | mood, partnership (partner+auth), homepage (missyou), bucket, game, drive |
| Polling fallback (15s) after 3 failures | `RealtimeSyncConnection.activatePollingFallback` → `_refreshAll` (realtime_connection.dart) | all except profile |
| Resume from background | `RealtimeSyncConnection` lifecycle observer → `_refreshAll` after reconnect (realtime_connection.dart) | all except profile |

## How state is persisted

- All feature data → `StorageService` (SharedPreferences-backed; documented in architecture scope) at every mutation success: moods (mood_state.dart:142,156,169,183,271-273), bucket list (bucket_state.dart:199), game question/history/matches (game_state.dart:122,173,202,224,236), drive items (drive_state.dart:122,139,176,241,258,271,292,316,326), homepage total (homepage_state.dart:50,66), profiles (profile_state.dart:68,77,91,114).
...
- Reauth-critical errors open the reauth dialog (error_handler.dart:145-146); API 401s call global `logout()` (auth_state.dart:81-86) and, because the synthetic `"401"` code is `requiresReauth`, also open the reauth dialog with a redirect to `/login`. Both leave feature data in memory untouched.
...
- **F-SM5** — Partner identity is duplicated across `AuthState` and `PartnerState` with separate refresh paths. `WHAT`: `_partnerId/_partnershipId` in AuthState vs `_partnershipInfo` in PartnerState; realtime updates both from the same source (lib/core/realtime/refetch_realtime_handler.dart), and `RealtimeSyncScope` must merge them (realtime_sync_scope.dart:46-53). `WHERE`: auth_state.dart:39-43, partner_state.dart:17-28, realtime_sync_scope.dart:46-53. `WHY`: Auth owned the identity for routing; Partner owned the full response; never unified. `WHEN`: simultaneous partnership transitions (accept in AuthState auth_state.dart:382-403 vs accept in PartnerState partner_state.dart:100-116). `IMPACT`: transient divergence, storage written twice; converges next frame. `CONFIDENCE`: MEDIUM.
- **F-SM6** — Profile staleness. `WHAT`: ProfileScreen uses neither TabScreenMixin nor (previously) RealtimeSyncService wiring (realtime_sync_scope.dart omitted the profile state). `WHERE`: profile_screen.dart:16 (plain StatefulWidget), realtime_sync_scope.dart:24-31. `WHY`: loadProfile uses cache+throttle instead (profile_state.dart:35-82). `WHEN`: partner changes profile, user stays on app. `IMPACT`: **PARTIALLY RESOLVED** — the realtime stack now subscribes `user_profiles` (`RefetchRealtimeHandler`, 150 ms debounce, update-only) and refreshes `ProfileState.loadProfile(force: true)` on self/partner updates; remaining gap is the no-refresh-on-tab-switch path for non-realtime stale states. `CONFIDENCE`: HIGH.
...
| Optimistic updates w/ rollback | IMPLEMENTED (drive: delete/toggle rollback, serialized cache writes; mood: in-memory-only optimistic + full rollback; bucket deferred to realtime) |
| Realtime push into local state | IMPLEMENTED (`lib/core/realtime/` facade + connection + handlers; profile excluded) |
