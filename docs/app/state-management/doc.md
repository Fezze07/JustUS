...
- **F-SM6** — Profile staleness. `WHAT`: ProfileScreen uses neither TabScreenMixin nor (previously) RealtimeSyncService wiring (realtime_sync_scope.dart omitted the profile state). `WHERE`: profile_screen.dart:16 (plain StatefulWidget), realtime_sync_scope.dart:24-31. `WHY`: loadProfile uses cache+throttle instead (profile_state.dart:35-82). `WHEN`: partner changes profile, user stays on app. `IMPACT`: **PARTIALLY RESOLVED** — `RealtimeSyncService` now subscribes `user_profiles` (`_handleUserProfilesPayload`, 150 ms debounce) and refreshes `ProfileState.loadProfile(force: true)` on self/partner updates; remaining gap is the no-refresh-on-tab-switch path for non-realtime stale states. `CONFIDENCE`: HIGH.
