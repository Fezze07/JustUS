...
- Realtime-refreshed (F-SM6): `user_profiles` is subscribed via `UserProfilesRealtimeHandler` (handlers/user_profiles_realtime_handler.dart, 150 ms debounce); self/partner profile `UPDATE`s trigger `loadProfile(force: true)` on `ProfileState` plus partnership/partner refreshes. Regular (non-wipe, non-realtime) loads still depend on the homepage `_loadData` or explicit `loadProfile(force: true)`. Note `profile_screen.dart` does NOT use TabScreenMixin either.
...
- **F-SM5** — Partner identity is duplicated across `AuthState` and `PartnerState` with separate refresh paths. `WHAT`: `_partnerId/_partnershipId` in AuthState vs `_partnershipInfo` in PartnerState; realtime updates both from the same source (handlers/partnership_realtime_handler.dart), and `RealtimeSyncScope` must merge them (realtime_sync_scope.dart:46-53). `WHERE`: auth_state.dart:39-43, partner_state.dart:17-28, realtime_sync_scope.dart:46-53. `WHY`: Auth owned the identity for routing; Partner owned the full response; never unified. `WHEN`: simultaneous partnership transitions (accept in AuthState auth_state.dart:382-403 vs accept in PartnerState partner_state.dart:100-116). `IMPACT`: transient divergence, storage written twice; converges next frame. `CONFIDENCE`: MEDIUM.
- **F-SM6** — Profile staleness. `WHAT`: ProfileScreen uses neither TabScreenMixin nor (previously) RealtimeSyncService wiring (realtime_sync_scope.dart omitted the profile state). `WHERE`: profile_screen.dart:16 (plain StatefulWidget), realtime_sync_scope.dart:24-31. `WHY`: loadProfile uses cache+throttle instead (profile_state.dart:35-82). `WHEN`: partner changes profile, user stays on app. `IMPACT`: **PARTIALLY RESOLVED** — the realtime stack now subscribes `user_profiles` (`UserProfilesRealtimeHandler`, 150 ms debounce) and refreshes `ProfileState.loadProfile(force: true)` on self/partner updates; remaining gap is the no-refresh-on-tab-switch path for non-realtime stale states. `CONFIDENCE`: HIGH.
