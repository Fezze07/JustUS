...
| `_keyAccessToken` | `access_token` | `String?` | `AuthState.setLoginData`, `tokenRefreshed` listener, `ApiService._tryRefreshToken` | `AuthState.init`, `ApiService._getHeaders` reads from Supabase live session (NOT from secure storage) | Session lifetime | `deleteAll` | `deleteAll` | Written in 3 places; `_tryRefreshToken` also pushes the access+refresh pair into the live Supabase session via `setSession` (todo# 1.1); read in 1 (`AuthState.init` for bootstrap check only) |
| `_keyRefreshToken` | `refresh_token` | `String?` | `AuthState.setLoginData`, `tokenRefreshed` listener, `ApiService._tryRefreshToken` | `ApiService._tryRefreshToken` (manual refresh proxy) | Session lifetime | `deleteAll` | `deleteAll` | Also pushed into the live Supabase session via `setSession` by `_tryRefreshToken` (todo# 1.1) |
...
- **Dual token refresh (Supabase listener + `ApiService._tryRefreshToken`) writes to the same secure-storage keys concurrently.** Since todo# 1.1, `_tryRefreshToken` also pushes the refreshed pair into the live Supabase SDK session via `setSession` (so the retried request uses the new token), which means both refresh paths now converge on the same live session. The `_isRefreshing` flag in `ApiService` prevents duplicate proxy refreshes, but does not prevent the Supabase built-in listener from refreshing simultaneously. Both still target the same keys (`access_token`, `refresh_token`); the Supabase listener at `auth_state.dart:157–164` and `ApiService._tryRefreshToken` at `api_service.dart:279–317` can race. The remaining concurrency race is tracked as todo# 1.6 / architecture finding F8.
