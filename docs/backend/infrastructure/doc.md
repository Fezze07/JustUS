...
|---|---|---|---|---|
| **PostgreSQL `pg_cron` vs. Node Retention** | `SKILL.md` mentions database cron job `cleanup_old_logs` (30 days retention). | Node.js `retentionJob.js` executes daily deletion with `LOG_RETENTION_DAYS` (default **90 days**); the SQL function is `cleanup_old_logs(p_days DEFAULT 90)` (aligned 2026-09-23) and not `pg_cron`-scheduled. | **NONE (RESOLVED)** | Node `retentionJob.js` is the single active driver; the SQL function shares the same 90-day default and can no longer truncate logs early. |
